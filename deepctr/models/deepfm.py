...
def DeepFM(linear_feature_columns, dnn_feature_columns, fm_group=(DEFAULT_GROUP_NAME,), dnn_hidden_units=(256, 128, 64),
           ...
           dnn_activation='relu', task='binary', dnn_use_bn=False, use_line=True, use_fm=True):
    ...
    logit_list = []
    if use_line:
        logit_list.append(linear_logit)
    if use_fm:
        logit_list.append(fm_logit)

    final_logit = add_func(logit_list + [dnn_logit])
